...
extern PetscInt  les, poisson,rans;
...
{ // All the vectors, nvert,ucont,P etc are initialized to zero.
  ...
  if(fdf){
    PetscPrintf(PETSC_COMM_WORLD," Particle Vector Initialize \n");
    ParticleVectorInitialize(user,numParticles);
  
    // DMGlobalToLocalBegin(user->pda, user->ParticleVec, INSERT_VALUES, user->lParticleVec);
    // DMGlobalToLocalEnd(user->pda, user->ParticleVec, INSERT_VALUES, user->lParticleVec);
  }
...
// We thus have dimensions for the coarsest grid level.
  ...
  PetscPrintf(PETSC_COMM_WORLD,"bcs-x-y-z-%s,%s,%s \n",(char)xperiod,(char)yperiod,(char)zperiod);
  ...
  if (fdf && l == usermg->mglevels-1) { // pda for particlevec at the finest level
    DMDAGetInfo(user[bi].da, PETSC_NULL, &MM,&NN,&PP,&m, &n, &p, PETSC_NULL, PETSC_NULL,PETSC_NULL, PETSC_NULL,PETSC_NULL,PETSC_NULL);
 
    ierr=DMDACreate3d(PETSC_COMM_WORLD,xperiod,yperiod,zperiod,DMDA_STENCIL_BOX,user[bi].IM+1, user[bi].JM+1, user[bi].KM+1,
		    m,n,p,2,s, PETSC_NULL, PETSC_NULL, PETSC_NULL,&(user[bi].pda));

    if (ierr) SETERRQ1(PETSC_COMM_SELF,1, "problem creating PDA  for FDF-LES \n",ierr);
  }
  
  for (l=1; l<usermg->mglevels; l++) { // loop through all mg-levels finer than the coarsest level.
    user = mgctx[l].user;
  if(visflg)  PetscPrintf(PETSC_COMM_WORLD,"Field DA (fda) distribution determination to ensure processors share  the same grid locations for successive MG levels initiated\n");
  ...
  //  if (l==1) PetscPrintf(PETSC_COMM_WORLD, "Corresponing number of procs for coarsest DA in i,j and k direction are %d %d %d :\n",m,n,p);
    ...
    if (fdf &&  l == usermg->mglevels-1) { // pda for particles for ALE solver.
      ierr=DMDACreate3d(PETSC_COMM_WORLD,xperiod,yperiod,zperiod,DMDA_STENCIL_BOX,
		      user[bi].IM+1, user[bi].JM+1, user[bi].KM+1,
		      m, n, p,
		      2, s, lxSum, lySum, lzSum,
		      &(user[bi].pda));
      if (ierr) SETERRQ1(PETSC_COMM_SELF,1, "problem creating PDA for FDF-LES %d",ierr);
    }
 ...
 //-------- Grid Generation for coarser levels ---------------
  ...
  for (level=usermg->mglevels-1; level>=0; level--) {
	...
	VecDuplicate(user[bi].lAj, &user[bi].lUstar);
      ...
      // For FDF Vector Creation
      if(fdf && level == usermg->mglevels-1){
	//	ParticleVectorCreate(numParticles, user[bi]);
	DMCreateGlobalVector(user[bi].pda,&user[bi].ParticleVec);
        VecSet(user[bi].ParticleVec,0.0);
        
	DMCreateLocalVector(user[bi].pda,&user[bi].lParticleVec);
        VecSet(user[bi].lParticleVec,0.0);  

        PetscPrintf(PETSC_COMM_WORLD," particlevecs created \n");      
      }
  ...
  for (level=usermg->mglevels-1; level>=0; level--) {
	...
	if(rans==3) {
      ...
      if(fdf && level == usermg->mglevels-1){
        ...
        VecDestroy(&user[bi].ParticleVec);
        VecDestroy(&user[bi].lParticleVec);      
        DMDestroy(&user[bi].pda);
      }
